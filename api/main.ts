...
import { Pool, Client, Transaction } from "https://deno.land/x/postgres@v0.13.0/mod.ts";
...
function useTransaction<T>(fn: (transaction: Transaction) => Promise<T>): Promise<T> {
...
router.post("/v1/products", async (context) => {
  ...
  const { name, price, count } = await context.request.body({ type: "json" }).value;
  if (name == null || price == null || count == null) {
    context.response.status = 400;
    context.response.body = JSON.stringify({ errors: ["Parameter 'name' or 'price' or 'count' not found."] });
    return;
  }
  ...
  await useTransaction(async (transaction) => {
    ...
    if (name != null || price != null) {
      ...
      if ((latestData.rowCount ?? 0) === 0) {
        context.response.status = 404;
        context.response.body = JSON.stringify({ errors: [`Product id(${id}) is not found.`] });
        return;
      }
...
router.post("/v1/users", async (context) => {
  ...
  if (name == null) {
    context.response.status = 400;
    context.response.body = JSON.stringify({ errors: ["Parameter 'name' not found."] });
    return;
  }
